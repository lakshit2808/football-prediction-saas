...
# API
load_dotenv()
api_key = dotenv_values(".env")["STATS_API_KEY"]
...
date_of_match = st.sidebar.date_input('Date of Match', key='Date')
total_articles = st.sidebar.number_input('Total Articles', 5)
num_posts = st.sidebar.number_input('Number of Posts', 3)
...
#     st.experimental_rerun()  # Optional: to refresh the page or redirect
# import streamlit as st
# import pandas as pd
# from socialmedia import RedditPredictor
# from news import ArticleAnalyzer
...
# # Streamlit app setup
# st.title('Football Match Prediction Inputs')

# # Inputs
# home_team = st.text_input('Home Team', '')
# away_team = st.text_input('Away Team', '')
# date_of_match = st.date_input(f'Date of Match', key=f'Date')

# st.subheader('Data Collection Input Parameters')
# total_articles = st.number_input('Total Articles', 5)
# num_posts = st.number_input('Number of Posts', 3)
...
#         total_steps = 4  # Total number of processes
#         progress = [0]   # Use a list to hold the progress value
...
#         #         article_summary = analyzer.summarize_articles(article_contents)
...
#             st.stop()     
#         # try:
#         #     with st.spinner("Summarizing Fans View..."):
#         #         fan_summary = predictor.summarize_comments(reddit_data)
#         #         st.success("Summarizing completed!")
#         #         update_progress()
#         # except Exception as e:
...
# else:
#     st.write("*NOTE:-* Please fill in all input fields to enable the **Prediction** button.")
